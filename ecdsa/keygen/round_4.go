...
func (round *round4) Start() *tss.Error {
	...
	for j, msg := range round.temp.kgRound3Messages {
		...
		go func(prf paillier.Proof, j int, ch chan<- bool) {
			...
			if err != nil {
				common.Logger.Println(round.WrapError(err, Ps[j]).Error())
				ch <- false
				return
			}
			ch <- ok
	...
	for j, ok := range round.ok {
		if !ok {
			...
			common.Logger.Printf("paillier verify failed for party %s", Ps[j])
			continue
		}
		common.Logger.Printf("paillier verify passed for party %s", Ps[j])
